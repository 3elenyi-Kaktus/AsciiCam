...
#include "string"
#include "memory"
#include "windows/video_gallery.h"
...
void EnterVideo(int option, Terminal &terminal, WebCamera &camera, GUI &interface) {
    const std::vector<std::string> s = {"tech", "chat", "video"};
    Network network(s);
    if (network.Connect() < 0) {
        return;
    }
    if (option == 1) {  // host
        ...
        while (true) {
            auto coords = PrintInputMenu(terminal.screen, interface.passcode_enter);
            std::string input = GetInputFromInputMenu(coords);
            logger << "Entered passcode: " << input << "\n";
            network.SendMessage("tech", input); // send entered passcode to server for confirmation
    ...
    int height = terminal.screen.getSize().height;
    int width = terminal.screen.getSize().width - 4;
    int companion_height, companion_width;
    int companion_new_height, companion_new_width;
    int other_side_has_camera;
    ...
    logger << "Companion's camera is " << ((other_side_has_camera) ? "" : "not ") << "initialized\n";
    if (camera.is_initialized) { // calculate to which size frames should be transformed before sending to peer
        cv::Mat frame = camera.GetNewFrame();
        std::pair<int, int> ret_size = camera.GetFittedFrameSize(frame, companion_height, companion_width);
        ret_size = ConvertSizeToSymb(ret_size);
        companion_new_height = ret_size.first;
        companion_new_width = ret_size.second;
        logger << "Companion's frame resulting size is height x width: " << companion_new_height << "x"
    ...
    msg = std::to_string(camera.GetNewFrame().rows) + " " + std::to_string(camera.GetNewFrame().cols);
    network.SendMessage("tech", msg);

    network.GetMessage("tech", reply);
    translator.clear();
    translator.str(*reply);
    translator >> height >> width;
    logger << "\n\nABRACADABRA resulting height x width: " << height << "x" << width << "\n";
//     Enter get/send state, in which in 2 threads:
    // 1. Get frame, Convert frame, Send frame
    // 2. Get other persons frame, Print it
    // Threads are activated only if their existence is appropriate (? not anymore)
...
//    std::mutex mutex;
    ...
    std::thread receiver = std::thread([&]() {
        logger << "Created receiving thread\n";
        Coordinates coords;
        VideoGallery gallery(coords, terminal.screen.getSize(), terminal.screen, camera);
        while (true) {
            std::unique_ptr<std::string> reply;
            if (network.GetMessage("video", reply) < 0) {
                break;
...
//                std::cerr << reply->length() << "\n";
//                std::cerr << *reply << "\n\n\n\n";

                for (int i = 0; i < 480; ++i) {
                    for (int j = 0; j < 640; ++j) {
                        mat.at<cv::Vec3b>(i, j)=cv::Vec3b((*reply)[i * 640 * 3 + j * 3],
                                                          (*reply)[i * 640 * 3 + j * 3 + 1],
                                                          (*reply)[i * 640 * 3 + j * 3 + 2]);
                    }
                }
//            std::memcpy(A.data, reply->c_str(), reply->length());

                gallery.updateWindow(mat);
            } else {
                std::cerr << "\n\nGOT COULD BE FRAME IN RECEIVER\n\n\n";
            }
    ...
    std::thread sender = std::thread([&]() {
        ...
        while (true) {
            ...
            if (camera.is_initialized) {
                cv::Mat frame = camera.GetNewFrame();
//                std::cerr << frame.type() << " - oldtype\n";
//                cv::cvtColor(frame, frame, cv::COLOR_RGB2GRAY);
//                std::cerr << frame.type() << " - newtype\n";
                frame = (frame.reshape(0,1));
                int imgSize = frame.total()*frame.elemSize();

                msg = std::string(frame.data, frame.data + imgSize);
                std::cerr << "Msg len: " << msg.length() << "\n";
...
void Benchmarking(Terminal &terminal, WebCamera &camera) {
    Coordinates lol;
    std::shared_ptr<Chat> chat = std::make_shared<Chat>(lol, terminal.screen.getSize(), terminal.screen);
    terminal.active_windows.push_back(chat);
...
//    std::cerr.rdbuf(file.rdbuf()); // <----
    ...
    WebCamera camera;
    while (true) {
        auto coords = PrintMenu(terminal.screen, interface.menu);
        ...
        if (option == 1) {
            coords = PrintMenu(terminal.screen, interface.CS_choice);
...
//                EnterVideoChat(option, terminal, camera, interface);
            ...
            if (option == 3) {
                ...
                EnterVideo(option, terminal, camera, interface);
